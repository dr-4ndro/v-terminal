...
# ============================================
#  OPSEC Suite – anon.sh
#  Usage: anon {start|stop|restart|change|status}
# ============================================
...
anon_start() {
    ...
    if ! command -v tor &>/dev/null; then
        echo -e "  ${BOLD}${RED}[-]${RESET} Tor is missing. Run the v_terminal setup again."
        return 1
    fi
    ...
    local iface=$(ip -o link show 2>/dev/null | grep -v "lo:" | awk -F': ' '{print $2}' | head -1)
    if [ -n "$iface" ]; then
        macchanger -r "$iface" 2>/dev/null || echo -e "  ${YELLOW}[!]${RESET} MAC spoof needs root."
    else
        echo -e "  ${YELLOW}[!]${RESET} No network interface found."
    fi

    local new_host="anon-$(printf '%04x%04x' $RANDOM $RANDOM)"
    echo "$new_host" > "$HOME/.custom_hostname"
    export HOSTNAME="$new_host"
    echo -e "  ${BOLD}${GREEN}[√]${RESET} Hostname set: ${BOLD}${RED}${new_host}${RESET}"

    echo ""
    echo -e "${BOLD}${GREEN}[√]${RESET} Anonymous mode is active."
    echo -e "  • Use : ${BOLD}proxychains <command>${RESET}"
    echo -e "  • Check: ${BOLD}anon status${RESET}"
...
case "$1" in
    ...
    change)   anon_change ;;
    status)   anon_status ;;
    *)        echo -e "${BOLD}${CYAN}Usage: anon {start|stop|restart|change|status}${RESET}" ;;
esac    echo -e "\n${BOLD}${GREEN}[+]${RESET} Starting Anonymous Mode...\n"

    # Check that required tools exist (should have been installed by terminal.sh)
    if ! command -v tor &>/dev/null; then
        echo -e "  ${BOLD}${RED}[-]${RESET} Tor is missing. Run 'pkg install tor' first."
        return 1
    fi
