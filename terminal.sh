...
#  Features: Font, Banner, Venv Prompt, OPSEC
#  OPSEC: Tor, ProxyChains, MAC Spoof, DNS
# ============================================

# Animation function (Spinner)
spin() {
    local pid=$1
    local msg="$2"
    local delay=0.1
    local spinstr='|/-\'
    echo -ne "${BOLD}${GREEN}[+]${RESET} ${msg}  "
    while kill -0 "$pid" 2>/dev/null; do
        for (( i=0; i<${#spinstr}; i++ )); do
            printf "\r${BOLD}${GREEN}[%c]${RESET} ${msg}" "${spinstr:$i:1}"
            sleep "$delay"
        done
    done
    printf "\r${BOLD}${GREEN}[√]${RESET} ${msg} done.   \n"
}
