...
# ============================================
#  v_terminal Setup Script (Interactive Fix)
#  Must be run as: bash <(curl -sL <URL>)
# ============================================
...
# ------------------------------------------------------------
#  Ask for custom username (interactive – will work because
#  stdin is terminal when using bash <(curl ...))
# ------------------------------------------------------------
...
if [ -f "$FONT_FILE" ] && [ -s "$FONT_FILE" ]; then
    echo -e "${BOLD}${GREEN}[i]${RESET} Font already installed. Skipping."
else
